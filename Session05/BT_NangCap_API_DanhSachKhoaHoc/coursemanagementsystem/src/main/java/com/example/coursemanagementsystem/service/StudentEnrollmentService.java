...
public class StudentEnrollmentService {
    ...
    // New specific business methods for BT_PhatTrien_NghiepVu_QuanLySinhVienTrongKhoaHoc

    public CourseEnrollmentResponse enrollStudentInCourse(Long courseId, CourseEnrollmentRequest request) {
        if (!courseRepository.existsByIdAndStatus(courseId, CourseStatus.ACTIVE)) {
            throw new IllegalArgumentException("Course with ID " + courseId + " is not ACTIVE or does not exist.");
        }

        if (studentEnrollmentRepository.existsByCourseIdAndStudentId(courseId, request.getStudentId())) {
            throw new IllegalArgumentException("Student with ID " + request.getStudentId() + " is already enrolled in course with ID " + courseId + ".");
        }

        Student student = studentRepository.findById(request.getStudentId())
                .orElseThrow(() -> new IllegalArgumentException("Student with ID " + request.getStudentId() + " does not exist."));
    ...
    public void dropoutStudentFromCourse(Long courseId, Long studentId) {
        StudentEnrollment enrollment = studentEnrollmentRepository.findByCourseIdAndStudentId(courseId, studentId)
                .orElseThrow(() -> new IllegalArgumentException("Enrollment not found for student with ID " + studentId + " in course with ID " + courseId + "."));
    ...
    public List<CourseEnrollmentResponse> searchStudentsInCourse(Long courseId, String search) {
        ...
        List<StudentEnrollment> list = studentEnrollmentRepository.searchStudentsInCourse(courseId, search);

        return list.stream()
                .map(se -> new CourseEnrollmentResponse(
                    se.getStudent().getId(),
                    se.getCourse().getId(),
                    se.getEnrolledAt()
                ))
